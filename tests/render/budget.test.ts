...
it('backend sample request reaches the target policy',async()=>{const {profileWithSamples}=await import('../../src/render/RendererBackend');const profile=profileWithSamples(PROFILES.high,2);expect(planTargets(960,600,1,profile,caps).samples).toBe(4);expect(planTargets(960,600,1,profile,{...caps,backend:'webgl2',maxSamples:2}).samples).toBe(2);expect(profileWithSamples(PROFILES.high,1).msaaSamples).toBe(0);});
